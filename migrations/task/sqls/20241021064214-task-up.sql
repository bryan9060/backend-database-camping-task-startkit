...
UPDATE "USER" 
SET ROLE = 'COACH' 
WHERE email IN('LEE2000@HEXSCHOOLTEST.IO', 'STARPLATINUM@HEXSCHOOLTEST.IO', 'MUSCLE@HEXSCHOOLTEST.IO');
...
select count(user_id) from "COURSE_BOOKING" cb
where user_id = (select id from "USER" where name = '王小明');
...
select
	...
	select
			COUNT(*)
	from "COURSE_BOOKING"
	where join_at notnull ) as remaining_credit
from "CREDIT_PURCHASE"
...
inner join "COACH_LINK_SKILL" on
	"COACH".id = "COACH_LINK_SKILL".coach_id
inner join "SKILL" on
